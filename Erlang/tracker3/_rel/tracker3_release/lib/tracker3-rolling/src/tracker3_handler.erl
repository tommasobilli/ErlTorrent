...
handle_get_request(Req, #state{op=Op} = State) ->
    {ok, Key} = application:get_env(tracker3, jwt_secret),
...
get_user_request(Req, State) ->
    ...
    try
        ...
        {ok, Recordfilename} = application:get_env(tracker3, users_file_name),
        ...
        BodyResp = io_lib:format(Body, [jsx:encode(PickedUsers)]),
        {BodyResp, Req, State}
    catch
...
post_user_request(Req, State) ->
    ...
    ok = dets:insert(records_db, {Filename, Address}),
